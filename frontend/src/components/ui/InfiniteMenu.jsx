...
void main() {
    ...
    float imageAspect = texSize.y > 0 ? float(texSize.x) / float(texSize.y) : 1.0;
...
class InfiniteGridMenu {
  ...
  #initTexture() {
    ...
    // Initialize with 1x1 solid cyan pixel to ensure the texture is immediately complete/renderable in WebGL
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, new Uint8Array([0, 245, 255, 255]));
        ...
        item =>
          new Promise(resolve => {
            ...
            img.onerror = () => {
              ...
              ctx.fillText(item.title || 'NEXUS', 256, 256);
              resolve(canvas);
            };
            img.src = item.image;
          })
        ...
        <>
          <h2
            style={{
              ...
              left: "1.6em",
              top: "50%",
              transform: isMoving ? "translate(10%, -50%)" : "translate(20%, -50%)",
          ...
          >
            <p style={{ userSelect: "none", position: "relative", color: "#120F17", top: 1, fontSize: 26, margin: 0 }}>&#x2197;</p>
          </div>
        </>
      )}
    </div>
